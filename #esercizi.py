...
for i in range (N_toy):
    ...
    least_squares = LeastSquares (x_values2, y_values, sigma_y, phi)
    Q_squared = my_minuit.fval
    Q_due.append(Q_squared)
#print (Q_due)

xMin = min(Q_due)-10
xMax = max(Q_due)+10
nBins = floor (len (Q_due) / 10.) + 1     # number of bins of the histogram
bin_edges = np.linspace (xMin, xMax, nBins + 1)  # edges o the histogram bins
fig, ax = plt.subplots ()
ax.set_title ('Histogram of Q2 over numbers of ' + str (N_toy) + ' toys fits', size=14)
ax.set_xlabel ('Q2')
ax.set_ylabel ('toys in bin')
ax.hist (Q_due,      # list of numbers
             bins = bin_edges,
             color = 'orange',
             # normed = True,
            )
